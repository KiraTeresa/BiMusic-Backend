...
router.get("/", isLoggedIn, async (req, res) => {
    ...
    await Project.find({
    ...
    }).then(async (projectsArr) => {
        allProjects = projectsArr

        for (const proj of allProjects) {
            await Chat.findOne({
                project: Types.ObjectId(proj._id)
            }).populate('project').then((chatFound) => {
                if (chatFound) {
        ...
        message: "Error occured when trying to load chat list.",
        err
    }))
})
...
router.post("/", isLoggedIn, async (req, res) => {
    ...
    if (!req.body.newChat) {
        res.status(400).json({
            message: "You need to select a project in order to create the chat room."
        })
    }
    ...
    if (!isInitiator) {
        res.status(400).json({
            message: "You need to be the initiator of the project in order to create the chat room."
        })
    }
    ...
    if (Object.keys(newChat).length !== 0) {
        await Chat.findOne({
            project: Types.ObjectId(newChat)
        }).then(async (chatFound) => {
            if (chatFound) {
                res.status(400).json({
                    message: "The chat for this project already exists."
                })
            } else {
                await (await Chat.create({
                    project: Types.ObjectId(newChat)
                })).populate('project').then((chat) => {
        ...
        res.status(400).json({
            message: "Please select a project in order to create a chat."
        })
    }
})
...
router.get("/:chatId", isLoggedIn, async (req, res) => {
    ...
    await Chat.findById(chatId).populate({
        ...
        if (!isInitiator && !isCollab) {
            res.status(400).json({
                message: "You need to be a collaborator in order to join the chat."
            })
            return
        }
        ...
        message: "Error when finding the chat data ",
        err
    }))
})
