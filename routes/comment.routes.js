...
router.post("/:projectId", isLoggedIn, async (req, res) => {
    ...
    if(!text){
        return res.status(400).json({message: "Forgot to type the actual comment?"})
    }
    ...
    await Comment.create({text, author: req.user, project: Types.ObjectId(projectId)}).then(async (result) => {
        ...
        await Project.findByIdAndUpdate(projectId, {"$push": {comments: result}}, {"new": true}).then(() => {
            return res.json("Comment successfully added.")
        })
